...
class Orders(object):
    ...
    def email_results(self, filename):
        ...
        try:
            ...
            message.attach(MIMEText("Dear Team,\nAttached is today's Purchase Requisition Lines report."))
    ...
    def get_new_agent_orders(self):
        ...
        try:
            sql = ("""
                    SELECT
                        pt.name AS product, line.product_qty AS qtyordered,
                        line.qty_available AS qtyonhand, line.pending_out AS pendingorders,
                        line.qty_to_purchase AS topurchase,  line.category AS category
                    FROM product_product p
                    LEFT JOIN purchase_requisition_line line
                        ON line.product_id = p.id
                    LEFT JOIN product_template pt
                        ON pt.id = p.product_tmpl_id
                    WHERE line.create_date >= %s
    ...
    def format_xlsx(self, orders):
        ...
        headers = ['Product', 'Qty Ordered',  'Qty on Hand', 'Pending Orders', 'Qty To Purchase', 'Category']           
        empty_field = ['', '', '', '', '', '',]

        row_num = len(headers)
        # cel_merge = 'A1:K1'
        # active_sheet.merge_cells(cel_merge)
        active_sheet.append(['Purchase Requisition Lines'])
        ...
        result_file.save('purchase_requisition_lines.xlsx')
