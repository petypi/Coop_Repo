...
class ReceivedOrders(object):
    ...
    def email_results(self, filename):
        ...
        try:
            ...
            message.attach(MIMEText("Dear Team,\nAttached is today's Purchase Orders and their received quantities."))
    ...
    def get_orders(self):
        ...
        # orderDate = datetime.strptime("2018-03-10", "%Y-%m-%d").date()
        try:
            sql = '''select wh.name Warehouse, prt.name Supplier,po.name
                     OrderNumber,po.date_order OrderDate, po.date_planned DeliveryDate,
                     ptl.default_code ItemNumber,ptl.name ItemName, pol.product_qty QtyOrdered,
                     sm.product_qty QtyReceived from purchase_order po 
                     left join purchase_order_line pol on po.id = pol.order_id 
                     left join product_product prd on pol.product_id = prd.id 
                     left join product_template ptl on ptl.id = prd.product_tmpl_id
                     left join res_partner prt on po.partner_id = prt.id 
                     left join stock_picking_type spt on spt.id = po.picking_type_id
                     left join stock_warehouse wh on spt.warehouse_id = wh.id 
    ...
    def format_xlsx(self, OrdersToday):
        ...
        headers = ['Warehouse', 'Supplier',                                     
                   ...
                   'Item Name','Quantity Ordered', 'Quantity Received']    
