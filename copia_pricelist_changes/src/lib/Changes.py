...
class Changes(object):
    ...
    def get_pricelist_changes(self):
        """
        Get Products whose prices have been changed
        """
        agents = None
        try:
            sql = '''select alog.write_date, pp.name, alog.name, fld.name, 
                    alogl.old_value, alogl.new_value, rp.name 
                    ...
                    left join product_pricelist_item ppi on ppi.id = alog.res_id
                    ...
                    and alog.write_date >= (current_date - integer %s) 
                    order by alog.write_date desc;''' 
            params = ('Pricelist item', str(APP_SETTINGS['period']), )
            agents = self.db_management.retrieve_all_data_params(sql, params)
        except psycopg2.Error:
            raise
        return agents

    def format_xlsx(self, changes):
        """
        Turn the received dictionary into an xlsx file to be sent out
        """
        result_file = Workbook()
        active_sheet = result_file.active
        headers = ['Date Changed', 'Pricelist', 'Product', 'Field', 'Old Value', 'New Value', 'Changed By']
        active_sheet.append(headers)
        for change in changes:
            change = ["'" + str(item) if type(item) == datetime else item for item in change]
            active_sheet.append(change)
        result_file.save('pricelist_changes.xlsx')
